...
function renderCurrentWeather(dados) {
  ...
  if (sensacaoTermicaElement)
    sensacaoTermicaElement.innerHTML = `Sensação térmica: ${Math.floor(
      dados.main.feels_like
    )}°C`;

  const velocidadeVentoElement = document.querySelector(".velocidade-vento");
  if (velocidadeVentoElement)
    velocidadeVentoElement.innerHTML = `Vento: ${dados.wind.speed} m/s`;

  const pressaoAtmElement = document.querySelector(".pressao-atm");
  if (pressaoAtmElement)
    pressaoAtmElement.innerHTML = `Pressão: ${dados.main.pressure} hPa`;
}
...
async function renderDailyForecasts(forecastData) {
  ...
  forecastData.list.forEach((item) => {
    ...
    dailyData[dayKey].iconCounts[icon] =
      (dailyData[dayKey].iconCounts[icon] || 0) + 1;
  });
  ...
  daysToShow.forEach((dayKey) => {
    ...
    forecastCard.innerHTML = `
            ...
            <img src="https://openweathermap.org/img/wn/${
              data.mainIcon
            }@2x.png" alt="Ícone do tempo">
            <p class="temp-min-max">
                Min: ${Math.round(data.minTemp)}°C / Max: ${Math.round(
      data.maxTemp
    )}°C
            </p>
        `;
...
function getLocalizacaoAutomatica() {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Erro ao obter nome da cidade: ${response.statusText}`
            );
          }
          ...
          if (data && data.name) {
            ...
            throw new Error(
              "Não foi possível obter o nome da cidade para sua localização."
            );
          }
        } catch (error) {
      ...
      (error) => {
        ...
        switch (error.code) {
          ...
          case error.POSITION_UNAVAILABLE:
            errorMessage +=
              " Localização indisponível. Por favor, digite sua cidade.";
            break;
          case error.TIMEOUT:
            errorMessage +=
              " Tempo esgotado ao tentar obter localização. Por favor, digite sua cidade.";
            break;
          default:
    ...
    if (mensagemErroElement) {
      mensagemErroElement.textContent =
        "Geolocalização não é suportada por este navegador. Por favor, digite sua cidade.";
      mensagemErroElement.style.display = "block";
    }
    buscarCidade("São Paulo");
  }
